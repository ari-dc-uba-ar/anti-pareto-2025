...
const getAlumno = async (req: Request, res: Response): Promise<void> => {
  try {
...
const createAlumno = async (req: Request, res: Response): Promise<void> => {
  try {
...
const updateAlumno = async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
...
const deleteAlumno = async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ error: '${tablename} no encontrado' });
      return;
    }
    
    res.json({ message: '${tablename} eliminado correctamente' });
  } catch (error) {
...
return {getAlumno, getAlumnos, createAlumno, updateAlumno, deleteAlumno}
